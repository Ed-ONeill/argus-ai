...
export interface Consensus {
  pct: number; label: "Bullish" | "Bearish" | "Mixed"; color: string;
...
export function buildCollectiveIntel(
  ...
  if (!leadName) {
    ...
    const stance = cLabel === "Mixed" ? "split" : `leaning ${cLabel.toLowerCase()}`;
    read = `The tape is ${stance} (${pct}%) on ${leadName}`;
  ...
  return { read, consensus, widgets, wallStreet };
}
