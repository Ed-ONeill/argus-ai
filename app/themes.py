...
# ── Theme eligibility ───────────────────────────────────────────────────────────
# A "What Matters Now" card is a THEME, not a headline. A theme should exist only
# when ≥2 independent stories support the same narrative, OR the story clearly sits
# on a structural transmission path (the curated ontology). This stops isolated
# consumer/niche stories ("Municipal Bond Appeal", "Hospitality Sector Boost")
# from being promoted to themes.
_WMN_MIN_STORIES = 2

# Pre-compute, per ontology theme, its specific (non-generic) keywords and its
# entity set — used to decide whether a single story is on a structural path.
_STRUCTURAL_KEYWORDS: list[frozenset[str]] = []
_STRUCTURAL_ENTITIES: list[frozenset[str]] = []
for _cfg in THEME_ONTOLOGY.values():
    _generic = set(_cfg.get("generic_keywords", []))
    _STRUCTURAL_KEYWORDS.append(frozenset(
        k.lower() for k in _cfg.get("keywords", []) if k.lower() not in _generic
    ))
    _STRUCTURAL_ENTITIES.append(frozenset(e.upper() for e in _cfg.get("entities", set())))


def _on_structural_path(cluster: StoryCluster) -> bool:
    """
    True if the cluster's lead story maps to a curated structural theme — either a
    direct entity (ticker/company) hit, or ≥2 distinct specific (non-generic)
    keywords from the SAME theme. A single generic word ("ai") never qualifies.
    """
    p = cluster.primary
    text     = (getattr(p, "title", "") + " " + getattr(p, "snippet", "")).lower()
    entities = {e.upper() for e in (getattr(p, "affected_entities", None) or [])}
    for kw_set, ent_set in zip(_STRUCTURAL_KEYWORDS, _STRUCTURAL_ENTITIES):
        if entities & ent_set:
            return True
        hits = 0
        for kw in kw_set:
...
def _wmn_eligible(cluster: StoryCluster) -> bool:
    """A cluster may become a What-Matters-Now theme only if it is multi-story or
    structural — never an isolated one-off narrative."""
    if getattr(cluster, "story_count", 1) >= _WMN_MIN_STORIES:
        return True
    return _on_structural_path(cluster)
...
def select_what_matters_now(
    ...
    for s, cluster in scored:
        ...
        # Theme gate: only multi-story or structural narratives become themes.
        if not _wmn_eligible(cluster):
            log.debug(
                "[wmn] skipping isolated one-off (story_count=%d, non-structural): %.60s",
                getattr(cluster, "story_count", 1), getattr(cluster.primary, "title", ""),
            )
            continue
