...
| V1.3 | 2026-07-16 | **IR-1 shipped — the event kind in production as a RECORD (§2.2), the first consumer of the canonical Explanation (IRE-1).** Admission: `event:cluster:<id>` admitted in `lib/intel/dossier.ts` (exact cluster-id match, never transformed; malformed → invalid; other event namespaces → reserved); alias `/event/<id>` redirects to canonical `/intel/event:cluster:<id>`. Builder `buildEventDossier` is a pure projection of the admitted MarketEvent + its Explanation (typed section views `X*`; merged cluster ids resolve to the keeper). The research note (`components/intel/EventDossier.tsx`) renders Executive Summary (deterministic glue over the engine's sections, `derived`-chipped) → What Changed (delta; `unchanged` at full quality) → Why It Matters (position: theses, regime, attribution — no LLM prose anywhere on the page) → Evidence (tiered, qualified marks, evidence refs) → Transmission Chain (typed hops with rel UIDs, weakest-hop stated, legacy prose demoted to a labeled footnote) → Counterarguments (symmetric-search items or the coverage statement verbatim) → Confidence (band + factor decomposition + cap; no probability) → What to Watch Next (recorded thresholds + lane rules + the engine's gated sections verbatim). Engine statuses render as chips; non-available sections voice the ENGINE's note — the page consumes reasoning, never computes it. Not-in-cycle is a designed state (ids persist as archive evidence refs; durable event archive named as future work). Shared section primitives extracted to `components/intel/primitives.tsx` so file and record cannot drift (the shape test, enforced by code). Handoff: dossier event rows link "Research note →". Tests: intelTests 26/26 (admission, projection purity, developing lane, material delta + cap honesty, not-in-cycle, merged ids, determinism). |
| V1.2 | 2026-07-16 | **EI1.1 — attribution and legibility.** Event attribution is explicit and never inferred from theme membership: `MarketEvent.companies_direct` (backend; resolver-named companies recorded before the theme-asset merge — provenance un-lost, no new intelligence) + frontend attribution classes `direct / peer / industry_exposure / theme_exposure / relationship_exposure` (the last defined but never produced — no recorded inter-company relationship engine exists yet; admission law applies). The Event Record is direct-only; indirect items render under **Related Market Developments** with the reason stated verbatim ("Peer event: Micron Technology", "Theme exposure: AI Infrastructure", "Industry exposure: Semiconductors"); connections too weak to attribute are excluded; pre-EI1.1 cycles render "unattributed", never guessed. Standing View translated to investor language: each exposure answers *what the thesis means* (recorded description), *why this company connects* (because-clause + chain prose), *how it is known* (recorded/derived chip). Watch: deduplicated, company-specific questions composed from the recorded exposure chains; themes sharing a chain share one question citing both. Legibility: wider column (max-w-5xl), body text at document sizes (13.5/15.5px; micro ramp reserved for metadata), stronger masthead, proportional map height. |
