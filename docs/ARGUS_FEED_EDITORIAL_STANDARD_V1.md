...
| V1.3 | 2026-07-16 | F2 implemented — editorial judgment. Qualification: tier ≤2, or tier-3 specialist with specificity (named party + hard figures / quoted documents); specificity earns the tier-2 base treatment (E5 — substance, not masthead, upgrades authority); tier 4 never corroborates. Developing lane live: corroboration == 1 → `developing`, auto-promoted the cycle a second qualified source lands; aggregator echoes never promote. Admission floor (E8): ≥1 qualified source AND EventScore ≥ 10.0 — absolute, never flexed by supply; quiet day = short feed; price echoes expire below the floor by ~2h. One-event-once hardened: near-duplicate fold across clusters (same class + same companies + headline Jaccard ≥ 0.5, or company-less ≥ 0.7). Class weights re-pinned to desk order: macro/central-bank 30 > policy 26 > earnings 24 > M&A 22 > sector/market 12 > single-name 10 > price-echo 4; commentary suppressed upstream = never admitted. |
