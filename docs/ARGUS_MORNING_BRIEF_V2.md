...
1. **Sprint B1 - Brief view model (no UI).** `lib/morningBrief.ts`: a pure `buildMorningBrief(inputs)` composing the engines above into section-status-wrapped objects (`ProfileSection` pattern), with injected inputs (themes, clusters, backend memory payloads) per the established injection-not-fetching rule. A `useMorningBrief` hook provides graph build + `/api/memory/*` fetch + memoization. Tests 78.x: determinism, honest degradation, no blended narrative confidence, delta lines only from real memory payloads. **Also in B1: delete `UPCOMING_CATALYSTS` and its render** - fabrication does not wait for its replacement.
2. **Sprint B2 - Sections 4.1 + 4.5 + 4.7.** Wire the delta ledger (backend memory first), risks (three-kind split), and evidence regrouping into `BriefOpen`. LLM prose demotes per section 6. Acceptance: every rendered line traceable to an engine output; C2 confidence bar removed.
