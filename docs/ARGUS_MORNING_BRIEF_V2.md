...
3. **Sprint B3 - The Read. Done (2026-07-09):** `lib/theRead.ts` implements the Narrative Centerpiece zones Z2-Z7 (`buildTheRead`, composed into the brief VM as `vm.read`) - the first production consumer of `deriveNarratives`. The old summarizer core is gone from the render: Why Today Matters / Primary Narrative / Opportunities-Risks grid / Active Themes / the flat watch list are replaced by Z2 thesis (DERIVED NARRATIVE badge, member conviction chips with ThemeMemory trends, decomposed dominance, standing contradiction in-block; summarizer survives only as a quoted VOICE line), Z3 evidence spine (engine reads + cited stories), Z4 exposure map (deduped, member-counted, + commodity/FX/country neighbors), Z5 transmission chain (real recorded path walked layer by layer from the anchor driver, every hop a real edge, trends visible), Z6 watch-today with zone bindings, Z7 falsifiers (three kinds; recorded absence renders as an explicit warning). Every zone names its engine in its header; entities link to Explorer via the shared routing. Honest fallback: no derivable narrative renders a THEME-labeled single-theme read; no graph collapses chain/falsifiers to unavailable. Tests 80.x. Still open from the original B3 line: envelope demotion (C8) and marketing sections signed-out (C9) - moved to B4.
4. **Sprint B4 - Research Priority + Catalysts + Investigation Queue. Done (2026-07-09):** The Read completes as a decision-support system (`lib/theRead.ts`). **Research Priority** replaces the old "opportunities" framing: a ranked, capped attention list over recorded factors only (theme conviction, change-ledger deltas by kind, memory confirmations/contradictions, narrative membership, `rankFutureOpportunities` rank - first consumer of that engine, thin-profile signal from unresolved profile sections, exposure weight), every item fully decomposed in `reasons[]`, section marked partial ("heuristic ranking, not a trade signal"). Personalization: followed themes add a fixed +8 ordering boost, badged YOURS, reason appended - facts identical for every user (tested byte-for-byte). **Catalysts** surface only verified event material: MacroSeries/EconomicRelease nodes actually recorded against the thesis's drivers/members, explicitly dateless ("VERIFIED · NO DATE"); nothing recorded renders an honest absence naming the missing Event provider. **Investigation Queue** ends The Read with Explorer routes (Open Narrative / View Transmission / Inspect Evidence / Trace Relationships), each with its reason, entities deduped. Tests 81.x. Still open: envelope demotion (C8), marketing signed-out (C9), dated calendar (Event provider), Relevance Rail beyond priorities, bundle code-split pass.
