...
| D2 | ~~`sectorIntelligence` thesis/risk/driver templates + `SECTOR_ENTITIES`~~ **Resolved in P2.5 (2026-07-10):** the file shrank 1,153 -> ~200 lines. DELETED: `generateThesis`, `getCrossAssetEffects`, `getRiskFactors`, `getKeyDrivers` + `THEMATIC_DRIVERS`, `getPositioningNarrative`, `getLeadershipDynamics`, and the `INDUSTRY_DEVELOPMENTS`/leadership/positioning editorial dictionaries (~900 lines of sector prose wearing intelligence authority). RETAINED as classification-1 taxonomy: `SECTOR_ENTITIES`/`SECTOR_KEYWORDS` (ticker-to-sector membership + keywords, used exclusively for content FILTERING - they never render as winners/losers/risk/thesis), `filterSectorClusters`, `getMomentumState` (labeled current-state badge), cluster-derived `getLiveDevelopments`. Also removed from rendering: `industryConfig.geopoliticalExposure` and `maTheme` editorial prose, the fabricated "structural demand / capital cycle" transmission fallback rows, and Industries' `getEvolutionNarrative` fallback | profiles, graph exposure | done |
...
| D4 | ~~`watchlistIntelligence` snapshot store~~ **Resolved in P2.1 (2026-07-10):** `lib/watchlistIntelligence.ts` + `hooks/useWatchlistIntelligence.ts` deleted (the third memory system, localStorage key `argus:watchlist-snapshot`, its delta computation, alert chips, change sentences, and priorityScore); ThemeTerminal reads the canonical ledger | `intelligenceDeltas` + memory engines | done |
...
| D7 | ~~`listenIntelligence` matching + `episodeIntel` implications~~ **Resolved in P2.4 (2026-07-10):** `episodeIntel` REWRITTEN as a thin projection of shared engines (riskRead records verbatim, verified dateless catalysts / the canonical derived watch line, backend causal narrative as the only prose) - its keyword `catalystOf`/`riskOf` rules, templated implications, bull/bear cases, and rotating pseudo-labels deleted; `contradictingEpisodes` renamed `counterPositionedEpisodes` (a selection over canonical direction fields, explicitly NOT a contradiction record). Episode matching NARROWED, not deleted: `matchEpisodeThemes` stays as the deterministic metadata FALLBACK matcher (resolves to canonical theme keys; classified MENTIONS/CONTEXT, never SUPPORTS; prohibited from creating conviction or membership) because the graph's Podcast->Theme edges are not per-episode-specific (D14) | graph aliases, profiles, evidence engine | done (matcher documented as labeled fallback) |
...
| D11 | ~~the securities dictionary tail in `themeIntelligence`~~ **Resolved in P2.5 (2026-07-10):** `SECURITY_LIBRARY`, `secEntryFor`, `securitiesForSector`, `bestExpressions` (winWhy), and `themeLosers` (loseWhy) DELETED. `themeBeneficiaries` is now RECORDED data only (memory ticker sessions + pipeline related_assets; a theme with no recorded exposure shows no chips, honestly - 88.x pins it). Migrated consumers: Markets SectorPositioning exposures (pipeline assets only), ThemeDetailDrawer "Recorded Exposure" + "Most Exposed" (shared weakening edges, else recorded negative-weight sectors, basis-labeled), IntelligenceWorkspace who-benefits/who-is-hurt (recorded exposure + negative-weight sectors) | graph beneficiaries/weakening edges | done |
...
| D13 | **(registered 2026-07-10, P2.1; NARROWED in P2.4)** residual device-local user/alert stores: `useThemeAlerts` (`argus:theme-signals`) - **Listen migrated to canonical ledger records (2026-07-10); remaining consumers: Feed (`app/feed/page.tsx`) and Markets (`app/markets/page.tsx`) alert chips** - and the DUPLICATED theme-follow user state (`argus:theme-watchlist` ids vs `argus:followed-themes` id+name; Saved reads the union) | `intelligenceDeltas` (transitions); one follow store (user state) | `useThemeAlerts` retires globally once Feed + Markets chips move to ledger records; the two follow stores consolidate when alert preferences land - user state, not intelligence, so low drift risk |
| D14 | **(registered 2026-07-10, P2.4)** the canonical Listen adapter links every Podcast node to the GLOBAL matched-theme set plus its raw topics (`ingestListen` globalThemes), so Podcast->Theme graph edges carry no per-episode signal - forcing per-episode theme attribution to stay with the metadata fallback matcher | per-episode recorded Podcast->Theme edges (adapter emits only that episode's matches) | fix in the adapter with tests when graph shape can change safely (it alters narrative/evidence counts on every surface); until then the fallback matcher is the documented attribution home |
...
| **P2.1 Saved - DONE (2026-07-10)** | Saved is the user's STANDING INTELLIGENCE WATCH. `lib/savedIntel.ts` (`buildSavedIntel({savedEntities, followedThemes, themes, profiles, risks, deltas, narrativeOf, researchPriorities})`): a pure projection - filter/group/rank/annotate only, injected shared intelligence, no computation. Page wiring: `useArgusIntelligence` + per-entity `buildIntelligenceProfile`/`buildRiskRead` + `deriveMorningBriefDeltas` + The Read's priorities + `findNarrativeForTheme`; every monitored row shows conviction (pipeline/profile, basis named), ThemeMemory status/trend, the canonical ledger record VERBATIM (same object, no re-wording), shared risk + DERIVED watch, narrative membership, and routes into Explorer via `explorerHrefForNode`. Standing-watch summary = ledger + priorities restricted to the monitored set (not a scoring engine). `watchlistIntelligence` + its hook DELETED (D4); ThemeTerminal's watchlist chips/changes/ordering now read the canonical ledger (kinds rendered verbatim). **User state preserved (intentionally remains):** `useSaved` (story bookmarks, Supabase + localStorage `argus_saved_*`), `useWatchlist` (entities, Supabase + `argus_watchlist`), `useFollowedThemes` (`argus:followed-themes`), `useThemeWatchlist` (`argus:theme-watchlist`), device theme snapshots (`themeSnapshots` - an intelligenceDeltas INPUT, not a parallel interpreter). Saved-story cards keep their at-save fact snapshot (bookmark metadata, not live intelligence). Tests 86.x pin: Saved reads === profile/riskRead; changes ⊆ ledger (object identity); save/unsave changes inclusion+ordering only; honest degradation. Registered debt: D13. | D4 gone ✓; Saved deltas === brief ledger (86.x) ✓; Saved conviction === Explorer ✓; tsc/tests/build green ✓ |
...
| **P2.4 Listen - DONE (2026-07-10)** | Listen is the LONG-FORM EVIDENCE SURFACE: episodes are evidence attached to shared objects, never intelligence. `lib/listenIntel.ts` (`buildListenIntel({episodes, themes, read, risks, deltas, narrativeOf, researchPriorities, followedThemeNames, savedEntityIds})`): pure projection - attaches episodes to canonical entity keys, ranks, groups; creates no meaning. **Evidence classification model:** SUPPORTS (recorded supporting edge only), CONTRADICTS (recorded weakening edge), MENTIONS (recorded mention edge or entity-anchored metadata match), CONTEXT (topic/keyword match), UNCLEAR - a metadata match can NEVER become SUPPORTS (87.x pins it); episode cards badge RECORDED vs METADATA MATCH tiers. **Page:** `useArgusIntelligence` (canonical provisioning replaces the page's own feed query); new intelligence-led hierarchy above the browse catalog (`IntelLedSections`): A Relevant to Today's Read (dominant-narrative member themes, why-it-matters = the SAME thesis line, source badged), B New Evidence (canonical ledger records verbatim), C Contrarian/Contradicting Views (episodes on themes carrying ACTIVE evidence-engine contradiction / prediction invalidation records - shown verbatim, episode stance never inferred), D For Your Watch (followed/saved/priorities - ordering and inclusion only); every matched object routes into Explorer (`explorerHrefForNode`) or the ThemeDrawer. **episodeIntel rewritten** (D7): shared riskRead records, verified dateless catalysts / canonical watch line, backend causal narrative; invented implications/bull-bear/keyword rules deleted. **useThemeAlerts retired on Listen** (D13 narrowed): drawer alert badges are canonical ledger records. Retained as factual ingestion: entity/people extraction, earnings detection, speaker/show/company frequency aggregations, `useListenRails` content ranking; `listenConsensus`/`listenSections` conversation analytics remain surface-owned aggregation over canonical theme fields + mention counts. Tests 87.x (5 tests, 147/147 green). Registered debt: D14 (dense Podcast->Theme adapter edges). | D7 gone ✓; mention-never-support pinned ✓; why-listen = shared objects ✓; ledger alert badges ✓; tsc/tests/build green ✓ |
| **P2.5 Industries/Sectors - DONE (2026-07-10)** | Industries is the sector projection of the shared platform. `lib/industriesIntel.ts` (`buildIndustriesIntel({sectors, themes, profiles, risks, narratives, deltas, graphReady})`): pure projection per Sector entity - profile conviction/evidence/forward/drivers/beneficiaries, riskRead contradictions/invalidation/watch (verbatim), profile transmission strongestPath, DerivedNarrative exposure membership, canonical ledger record (object identity), profile evolution lines; `compareSectors` = pure filtering (comparison never recomputes, 88.x). **Detail page** (`industries/[slug]`): mounts `useArgusIntelligence`; What Matters Now = forward view + ledger record + narrative chip + Explorer handoff (honest partial states); Positioning = Forward View / What Breaks It / Watch (shared engines); Risk Factors = shared records; Cross-Asset Effects + Geopolitical Exposure prose sections DELETED, replaced by Recorded Transmission (profile strongest path, RECORDED GRAPH EDGES label); Leadership Dynamics = graph beneficiaries vs weakening edges; Drivers = recorded edges with stored macro-factor fallback (labeled); Industry Transmission Map = stored-field rows labeled, fabricated fallback rows deleted; evolution badges labeled current-state; `getEvolutionNarrative` fallback removed (its remaining consumer is M&A, retires P2.6). **Sectors page** (`/sectors`): builds the same VM per sector, `SectorIntelligenceCard` renders forward/narrative/ledger/risks/watch from shared reads (editorial thesis/cross-asset/risk generators deleted). D2 + D11 resolved (see register). Tests 88.x (3 tests, 150/150 green). Bundle: industries detail 285 -> 275 kB, list 199 -> 177 kB (editorial prose deleted). | D2, D11 gone ✓; sector conviction/drivers/beneficiaries === Explorer profile (88.x) ✓; ledger identity ✓; tsc/tests/build green ✓ |
