...
| A4 Entity-context VM | `buildEntityContext(entity)` - the drawer/FocusBar quick read as a profile projection; retires `buildForecast` and narrows `crossIntel` to narrative injection | Drawer, Feed story cards, Saved rows |
...
| D5 | `themeSignalDelta` change narrative + `themeEvolution`/`themeMomentum` as rendered intelligence | change ledger, ThemeMemory trends | P2.3; single-snapshot heuristics may survive only as badges, not narratives |
