...
| D13 | **(registered 2026-07-10, P2.1)** residual device-local user/alert stores: `useThemeAlerts` (`argus:theme-signals` - a signal-strength transition detector outside the ledger, used by Feed/Listen/Markets alert chips) and the DUPLICATED theme-follow user state (`argus:theme-watchlist` ids vs `argus:followed-themes` id+name; Saved reads the union) | `intelligenceDeltas` (transitions); one follow store (user state) | `useThemeAlerts` retires onto ledger records when Listen migrates (P2.4); the two follow stores consolidate when alert preferences land - user state, not intelligence, so low drift risk |
