...
| V1.1 | 2026-07-16 | **IRE-1 shipped — canonical backend Explanation assembly.** `app/explanations.py`: one Explanation per admitted MarketEvent, stages R0–R3 live (identity with EI1.1 attribution classes; evidence verbatim from the Wire desk with cluster-id evidence refs; position with typed transmission chains — every hop a recorded graph edge or curated ontology association carrying its rel UID; delta with the ±3 deadband and `unchanged` as a first-class answer; counter via symmetric search over recorded pressures edges + recorded trend deltas, absence stated as a coverage claim); confidence as a decomposed verdict band (weakest-hop cap, contradiction cap to moderate, probability always null); memory/stakes/falsifiers ship `gated` (reserved for IRE-2/4/5, humility note on falsifiers). Nine sections always present. **Section statuses use the IRE-1 delivery vocabulary** — `available, insufficient_evidence, not_applicable, conflicting, unavailable, gated, unchanged, developing` — mapping onto Part 5's states as: insufficient_evidence≈insufficient_signal, conflicting≈contested, unchanged≈quiet, not_applicable≈not_connected (data_gap folds into unavailable until a data-gap discriminator exists at the spine). Deterministic + content-hashed (reuses M3 `canonical_json`); built each pipeline cycle in `background.py` from the same deterministic narrative graph M3.2 records, cached on `ProcessedFeed.explanations`, served as `FeedResponse.explanations` keyed by event id. `MarketEvent.transmission_chain` (typed) added; `transmission` prose retained as LEGACY for compatibility. Frontend: types only (`Explanation`, `TransmissionHop` in types.ts); `evidenceEngine.ts` / `inferenceEngine.ts` marked LEGACY-PATH for output comparison during migration — no consumer UI changed. Tests: `tests/test_explanations.py` (16 — all eight required scenarios plus attribution, rel-UID preservation, graph-optional degradation, no-probability). |
